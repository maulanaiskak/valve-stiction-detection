...
Fixed in [valve-stiction-ml](https://github.com/maulanaiskak/valve-stiction-ml) via synthetic-data domain-randomization augmentation (train on a broad, physically-constrained range of similar signals, not the literal test config) plus a properly-derived decision threshold — see its `ML_PLAN.md` §15 for the full methodology, including a real label-quality bug a unit test caught along the way. Re-run against the same 255-window live streaming setup after the fix: **AUC 0.9998, 99.6% accuracy, F1 0.996** — now essentially matching the classic detector.

**Checked, not assumed, that this isn't just overfitting**: no train/validation leakage (group-based CV keyed by config, not window), and real SACAC performance is unchanged (0.865 ROC-AUC, same as before augmentation). But a genuine out-of-distribution test — signals with parameters well outside the trained ranges (`valve-stiction-ml/scripts/check_ood_generalization.py`) — scores AUC ~0.43-0.47, chance level. The model generalizes within the family of signals it was trained on, not beyond it; it did not learn a distribution-invariant concept of stiction. Both detectors still run unconditionally and are shown side by side on the dashboard regardless (see `HLD.md`'s NFR-1) — this fix closed the gap for this signal family, it doesn't make the RF's answer reliable outside it, so the classic detector stays the unconditional backstop.
